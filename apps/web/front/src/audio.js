    ...
    var successCallback = function(mediaStream) {
      ...
      var animation = function() {
        ...
        for( i = 0; i < idx4kHz; i++ ) {
          ...
          ctx.fillRect(x0, 0, barWidth, barHeight)
        }
        for( i = 0; i < noteList.length; i++ ) {
        ...
        var noteText = "---"
        if( currentNote.posValue > 2000 ) {
          // noteText = noteName[(currentNote.noteOffset + 60) % 12]
          noteText = noteName[(currentNote.noteOffset + 60) % 6]
          window.location.href = 'http://localhost:2300/pitchs'
        }
